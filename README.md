...
| `RETENTION_MAX_AGE` | unset | Optional. Keep events whose `created_at` is within this duration (e.g. `720h`, `2160h`). Unset = no age-based pruning. |
| `RETENTION_MIN_LEDGER` | unset | Optional. Keep events at or above this ledger (inclusive). Unset = no ledger-based pruning. |
| `RETENTION_BATCH_SIZE` | `5000` | Rows deleted per `DELETE` statement. Smaller = less lock pressure, slower. |
| `RETENTION_PAUSE` | `100ms` | Sleep between DELETE batches so ingestion stays responsive under load. |
| `RETENTION_INTERVAL` | `1h` | When nothing is eligible, how long to wait before the next sweep. |

See **Retention / pruning** below for behaviour, the safety guard, and metric exposure.
...
SoroTrail's retention feature has its own unit tests in `internal/pruner`,
covering batching, the never-delete-recent guard, the disabled-by-default
no-op, the combined age-plus-ledger policy, and the metrics snapshot. Run
just the pruner tests with `-race` enabled:

```sh
go test -count=1 -race ./internal/pruner/...
```
