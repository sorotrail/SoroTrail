...
| `CORS_ALLOWED_ORIGINS` | empty | Comma-separated list of allowed browser origins (scheme://host[:port]). Empty = no CORS headers (current behavior). Use `*` to allow any origin. |
